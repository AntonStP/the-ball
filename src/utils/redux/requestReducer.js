...
export function createCustomReducer({
  ...
  const thunk = customCreateAsyncThunk(sliceName, thunkName, func, controller, etc, options);

  return {reducer: initRequestReducer(thunk, {saveData, saveError, onSubmit, ...etc}, controller, thunkName), thunk}
}

export function customCreateAsyncThunk(sliceName, thunkName, request, controller, customData, options) {
